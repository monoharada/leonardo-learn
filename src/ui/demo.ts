...
import { BASE_CHROMAS } from "../core/base-chroma";
...
export const runDemo = () => {
	...
	// Helper: ステップ番号からインデックスを取得
	const stepToIndex = (step: number): number => {
		const stepNames = [
			50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950, 1000, 1050,
		];
		return stepNames.indexOf(step);
	};

	// Helper: クロマ名と強度からスケールを生成
	const generateScaleForChroma = (chromaName: string): Color[] => {
		// 基本クロマからHueを取得
		const chromaDef = BASE_CHROMAS.find((c) => c.name === chromaName);
		if (!chromaDef) return [];
	...
	const renderMain = () => {
		...
		// セマンティック・リンク・アクセントカラーセクションを追加
		renderExtendedColorSections(app);
	...
	// 拡張カラーセクションのレンダリング
	const renderExtendedColorSections = (container: HTMLElement) => {
		...
		// セマンティックカラーセクション
		renderSemanticColorSection(wrapper);

		// リンクカラーセクション
		renderLinkColorSection(wrapper);

		// アクセントカラーセクション
		renderAccentColorSection(wrapper);

		container.appendChild(wrapper);
	};

	// セマンティックカラーセクション
	const renderSemanticColorSection = (container: HTMLElement) => {
		const section = document.createElement("section");
		section.style.marginBottom = "2rem";

		const header = document.createElement("h2");
		header.textContent = "セマンティックカラー";
		...
		// Success
		const successGroup = createSemanticGroup(
			"サクセス",
			"Success",
			SEMANTIC_COLORS.filter((c) => c.category === "success"),
		);
		grid.appendChild(successGroup);

		// Error
		const errorGroup = createSemanticGroup(
			"エラー",
			"Error",
			SEMANTIC_COLORS.filter((c) => c.category === "error"),
		);
		grid.appendChild(errorGroup);

		// Warning (Yellow + Orange variant)
		const warningGroup = createWarningGroup();
		grid.appendChild(warningGroup);
		...
		note.textContent =
			"注意：上記のセマンティックカラーの例は、コントラスト比は白背景、またはそのカラーを背景色とした白文字を基準としています。背景色が異なる場合はその背景色でカラーコントラスト比を確認し、コントラスト比を保てるようカラー調整してください。";
	...
	const createSemanticGroup = (
		...
		label.innerHTML = `<span style="font-weight: bold;">${labelJa}</span><br><span style="color: #666;">${labelEn}</span>`;
		...
		colors.forEach((colorDef) => {
			const scale = generateScaleForChroma(colorDef.chromaName);
			const index = stepToIndex(colorDef.step);
			const color = scale[index];
			if (!color) return;

			const swatchWrapper = document.createElement("div");
			swatchWrapper.style.textAlign = "center";

			const swatch = document.createElement("div");
			swatch.style.width = "60px";
			swatch.style.height = "60px";
			swatch.style.backgroundColor = applySimulation(color).toCss();
			swatch.style.borderRadius = "4px";
			swatch.style.marginBottom = "0.25rem";
			swatchWrapper.appendChild(swatch);

			const name = document.createElement("div");
			name.textContent = colorDef.displayName;
			name.style.fontSize = "0.65rem";
			name.style.color = "#666";
			swatchWrapper.appendChild(name);

			const chromaStep = document.createElement("div");
			const chromaDef = BASE_CHROMAS.find(
				(c) => c.name === colorDef.chromaName,
			);
			chromaStep.textContent = `${chromaDef?.displayName || colorDef.chromaName}-${colorDef.step}`;
			chromaStep.style.fontSize = "0.6rem";
			chromaStep.style.color = "#999";
			swatchWrapper.appendChild(chromaStep);
	...
	// Warning グループ（Yellow + Orange の両方を表示）
	const createWarningGroup = () => {
		const group = document.createElement("div");

		const label = document.createElement("div");
		label.innerHTML = `<span style="font-weight: bold;">警告</span><br><span style="color: #666;">Warning</span>`;
		label.style.marginBottom = "0.5rem";
		label.style.fontSize = "0.85rem";
		group.appendChild(label);
		...
		warningContainer.style.gap = "0.5rem";

		// Yellow variant
		const yellowColors = SEMANTIC_COLORS.filter(
			(c) => c.category === "warning",
		);
		const yellowRow = createColorRow(yellowColors);
		warningContainer.appendChild(yellowRow);

		// 「又は」表示
		const orLabel = document.createElement("div");
		orLabel.textContent = "又は";
		orLabel.style.fontSize = "0.7rem";
		orLabel.style.color = "#999";
		orLabel.style.textAlign = "center";
		warningContainer.appendChild(orLabel);

		// Orange variant
		const orangeRow = createColorRow(WARNING_ORANGE_VARIANT);
		warningContainer.appendChild(orangeRow);

		group.appendChild(warningContainer);
		return group;
	};

	// カラー行を作成
	const createColorRow = (colors: typeof SEMANTIC_COLORS) => {
	...
	// リンクカラーセクション
	const renderLinkColorSection = (container: HTMLElement) => {
		const section = document.createElement("section");
		section.style.marginBottom = "2rem";

		const header = document.createElement("h2");
		header.textContent = "リンクカラー";
		header.style.fontSize = "1.2rem";
		...
		subHeader.textContent = "リンクテキストカラーの組み合わせスタイルの例";
		...
		table.style.gridTemplateColumns = "80px repeat(3, 1fr)";
		...
		LINK_COLOR_STYLES.forEach((style) => {
			const headerCell = document.createElement("div");
			headerCell.textContent = style.name;
		...
		states.forEach((stateInfo) => {
			...
			labelCell.textContent = stateInfo.label;
			...
			// 各スタイルの色
			LINK_COLOR_STYLES.forEach((style) => {
				const colorDef = style.colors.find((c) => c.state === stateInfo.key);
				if (!colorDef) {
				...
				if (color) {
					...
					swatch.style.backgroundColor = applySimulation(color).toCss();
					swatch.style.borderRadius = "4px";
					swatch.style.margin = "0 auto 0.25rem";
					cell.appendChild(swatch);

					const name = document.createElement("div");
					name.textContent = colorDef.displayName;
					name.style.fontSize = "0.6rem";
					name.style.color = "#666";
	...
	// アクセントカラーセクション
	const renderAccentColorSection = (container: HTMLElement) => {
		const section = document.createElement("section");
		section.style.marginBottom = "2rem";

		const header = document.createElement("h2");
		header.textContent = "アクセントカラー";
		header.style.fontSize = "1.2rem";
		header.style.marginBottom = "0.5rem";
		header.style.color = "#333";
		section.appendChild(header);
		...
		const subHeader = document.createElement("p");
		subHeader.textContent = "プリミティブカラーを用いたアクセントカラーの例";
		subHeader.style.fontSize = "0.85rem";
		subHeader.style.color = "#666";
		subHeader.style.marginBottom = "1rem";
		section.appendChild(subHeader);
		...
		ACCENT_COLORS.forEach((colorDef) => {
			const scale = generateScaleForChroma(colorDef.chromaName);
			const index = stepToIndex(colorDef.step);
			const color = scale[index];
			if (!color) return;
		...
		const note = document.createElement("p");
		note.textContent =
			"注意：アクセントカラーを装飾以外の用途や機能を持ったコンポーネントに使用する場合はコントラスト比の確保に留意してください。";
		note.style.fontSize = "0.75rem";
		note.style.color = "#666";
		note.style.marginTop = "1rem";
		section.appendChild(note);

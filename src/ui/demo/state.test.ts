...
describe("state module", () => {
	...
	describe("findPaletteByDadsInfo", () => {
		/**
		 * DADSトークン情報からパレット検索のテスト
		 * Issue #41: カラー詳細モーダルの統一
		 */

		it("should return undefined when no palettes exist", () => {
			expect(findPaletteByDadsInfo("Blue", 600)).toBeUndefined();
		});

		it("should return undefined when no matching palette exists", () => {
			const palette: PaletteConfig = {
				id: "harmony-accent1-1704067200000",
				name: "Accent (triadic 1)",
				keyColors: ["#ff0000"],
				ratios: [4.5],
			...
			// Blue 600 を検索（Redしかない）
			expect(findPaletteByDadsInfo("Blue", 600)).toBeUndefined();
		});

		it("should return undefined when step does not match", () => {
			const palette: PaletteConfig = {
				id: "harmony-accent1-1704067200000",
				name: "Accent (triadic 1)",
				keyColors: ["#ff0000"],
			...
			// Blue 700 を検索（600しかない）
			expect(findPaletteByDadsInfo("Blue", 700)).toBeUndefined();
		});

		it("should return matching palette when hue and step match", () => {
			const palette: PaletteConfig = {
				id: "harmony-accent1-1704067200000",
				name: "Accent (triadic 1)",
				keyColors: ["#00ff00"],
			...
			const result = findPaletteByDadsInfo("Green", 500);
			expect(result).toEqual(palette);
		});

		it("should return the most recent palette when multiple palettes match", () => {
			const older: PaletteConfig = {
				id: "harmony-accent1-1704067200000",
				name: "Accent (triadic 1) - older",
				keyColors: ["#0066cc"],
				ratios: [4.5],
				harmony: HarmonyType.TRIADIC,
				baseChromaName: "Blue",
			...
			const newer: PaletteConfig = {
				id: "harmony-accent1-1704067300000",
				name: "Accent (triadic 1) - newer",
				keyColors: ["#0066cc"],
				ratios: [4.5],
				harmony: HarmonyType.TRIADIC,
				baseChromaName: "Blue",
			...
			state.palettes.push(older, newer);

			const result = findPaletteByDadsInfo("Blue", 600);
			expect(result).toEqual(newer);
			expect(result?.id).toBe("harmony-accent1-1704067300000");
		});

		it("should handle palettes without baseChromaName", () => {
			const palette: PaletteConfig = {
				id: "harmony-brand-1704067200000",
				name: "Brand",
				keyColors: ["#ff0000"],
				ratios: [4.5],
				harmony: HarmonyType.NONE,
				// baseChromaName is undefined
			};
			state.palettes.push(palette);
		...
		it("should handle palettes without step", () => {
			const palette: PaletteConfig = {
				id: "harmony-brand-1704067200000",
				name: "Brand",
				keyColors: ["#ff0000"],
		...
		it("should handle Light Blue (multi-word hue name)", () => {
			const palette: PaletteConfig = {
				id: "harmony-accent1-1704067200000",
				name: "Accent (triadic 1)",
				keyColors: ["#b3e5fc"],
			...
			const result = findPaletteByDadsInfo("Light Blue", 200);
			expect(result).toEqual(palette);
		});

		it("should handle edge step values (50 and 1200)", () => {
			...
			const darkPalette: PaletteConfig = {
				id: "harmony-accent2-1704067200000",
				name: "Accent (darkest)",
				keyColors: ["#000000"],
				ratios: [4.5],
				harmony: HarmonyType.TRIADIC,
			...
			state.palettes.push(lightPalette, darkPalette);

			expect(findPaletteByDadsInfo("Gray", 50)).toEqual(lightPalette);
			expect(findPaletteByDadsInfo("Gray", 1200)).toEqual(darkPalette);
		});
	});

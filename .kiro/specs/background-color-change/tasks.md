...
- [x] 5.2 シェードビューに背景色を適用する
  - shades-view上部に背景色セレクターを配置
  - シェードプレビュー領域とシェードグリッドに背景色を適用
  - ホバー時のコントラスト表示にも新背景色を反映
  - NOTE: Req 3.5のホバー時コントラスト表示は、クリック時のモーダル内で現在の背景色に基づくコントラスト値が自動計算される
  - _Requirements: 1.1, 3.5, 5.5, 5.6_

- [x] 5.3 画面間での背景色同期を確認する
...
- [x] 6.3 スウォッチボーダーのモード対応と低コントラスト強調を実装する

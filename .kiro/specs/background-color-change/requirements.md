...
2. The システム shall 以下の入力方法を提供する:
   - カラーピッカー（視覚的な色選択）
   - HEX入力フィールド（#RRGGBB形式）
   - OKLCH入力フィールド（オプション、詳細モード）
3. When 無効な色値が入力された時, the システム shall エラーメッセージを表示し、前の有効な値を維持する
4. The システム shall 現在選択中の背景色をプレビュー表示する
...
3. The システム shall コントラストレベル表示（AAA/AA/L）を新しい背景色基準で更新する
4. The システム shall シェードビューのホバー表示でも新しいコントラスト値を反映する

...
- [x] 10. 統合 - 円形スウォッチ・欄外情報・コントラスト境界をシェードビューに統合する
...
- [x] 10.5 CVDシミュレーション時の表示を調整する（**実装完了**）
  - 欄外ロール情報のカテゴリ色は固定（シミュレーション適用外）→ ROLE_CATEGORY_COLORSから直接設定
  - コントラスト境界ピルの色は固定（シミュレーション適用外）→ CSSクラスで#333固定
  - スウォッチ自体のみシミュレーション色を表示→ applySimulation()で背景色に適用
  - 統合テスト: CVDモード時に欄外情報とピルの色が固定されること→ 6件のテスト追加
  - _Requirements: 4.3_

## Task 11: E2Eテストと品質保証

- [x] 11. 品質保証 - 新UIのE2Eテストとパフォーマンス検証

- [x] 11.1 旧E2Eテストを新UI仕様に更新する
...
- [x] 11.2 (P) 円形スウォッチのE2Eテストを実装する（**実装完了**）
  ...
  - 追加テスト: 各カテゴリのラベル表示（P/S/Su/E/W/L）、ラベル文字色の自動調整、複数ロール時の優先ロール表示
  - E2Eテスト: 13件追加、Unit: 1383件全パス
  - _Requirements: 2.1, 2.2, 2.3, 2.4, 2.6_

- [x] 11.3 (P) 欄外ロール情報のE2Eテストを実装する（**実装完了**）
  ...
  - E2Eテスト: 9件追加（欄外ロール情報表示テストスイート）
  - テストページ更新: renderRoleInfoBar/renderUnresolvedRolesBar追加
  - E2E 34件全パス、Unit 1383件全パス
  - _Requirements: 3.1, 3.2, 3.4, 3.5_

- [x] 11.4 (P) コントラスト境界のE2Eテストを実装する（**実装完了・Codexレビュー修正済み**）
  - コントラスト境界ピル（3:1→、4.5:1→、←4.5:1、←3:1）が正しいscale位置に表示されること
  - 白背景用ピルは白抜きスタイルであること
  - 黒背景用ピルは黒塗りスタイルであること
  - 4つの境界ラベルパターン全ての存在確認（6.2/6.3）
  - 各色相セクション全てで境界コンテナが存在することを確認（6.1）
  - スウォッチとの正確な位置関係を検証（6.5）
  - 全ハーモニータイプ（complementary/analogous/triadic/split-complementary）で非表示を確認
  - テストページ更新: calculateBoundaries/renderBoundaryPillsを追加
  - E2E: 14件追加（コントラスト境界表示テストスイート）
  - E2E 49件全パス、Unit 1383件全パス
  - _Requirements: 6.1, 6.2, 6.3, 6.4, 6.5_

- [x] 11.5 パフォーマンステストを更新する（**実装完了**）
  - マッピング計算 + コントラスト境界計算が200ms以内であること
  - DOM追加（円形スウォッチ + 欄外情報 + 境界ピル）によるレンダリングブロッキングがないこと
  - 既存パフォーマンステストを新UI構造に合わせて更新
  - **新規テスト追加（7件）**:
    - コントラスト境界計算パフォーマンス（10色相×13スケールが200ms以内）
    - マッピング + 境界計算合計パフォーマンス（200ms以内）
    - 高頻度コントラスト境界計算（1000回が1秒以内）
    - 欄外ロール情報バー生成（20アイテムが50ms以内）
    - 未解決ロールバー生成（10ロールが30ms以内）
    - コントラスト境界ピル生成（4ピルが20ms以内）
    - 全新UI要素統合DOM操作（1色相分が50ms以内）
  - Unit: 1396件パス（+7件追加）
  - _Requirements: 5.1, 5.2_

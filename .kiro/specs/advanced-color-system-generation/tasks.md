...
| **F: CVD Simulation** | 色覚シミュレーションと識別性検証 | 11.1-11.6 | 3-4日 |
...
| 10 | バージョニング | 9.1, 9.2 |
| 11 | 色覚シミュレーションと識別性検証 | 11.1, 11.2, 11.3, 11.4, 11.5, 11.6 |
...
- [ ] 11.1 (P) [F] CVDシミュレーターの基本変換を実装する
  - sRGB→linearRGB→LMS色空間への変換パイプラインを構築する
  - Protanopia、Deuteranopia、Tritanopia、Achromatopsiaの変換行列を実装する
  - Brettel 1997法またはMachado 2009法に基づいた正確な色変換を行う
  - LMS→linearRGB→sRGBへの逆変換を実装する
  - 変換精度の検証テストを作成する
  - _Requirements: 11.1, 11.2_

- [ ] 11.1.1 [F] CVDシミュレーターの単体テストを作成する
  - 既知の色に対するシミュレーション結果を検証する
  - 各色覚タイプでの変換が正しいことを確認する
  - _Requirements: 11.1, 11.2_

- [ ] 11.2 (P) [F] 色差計算と識別性検証ロジックを実装する
  - OKLCH空間でのΔE（ユークリッド距離）計算機能を実装する
  - シミュレーション後の色ペア間の色差を計算する
  - 閾値（ΔE < 3.0）以下の色ペアを「識別困難」として判定する
  - 重大度（warning/error）を判定するロジックを追加する
  - _Requirements: 11.3, 11.4_

- [ ] 11.2.1 [F] 色差計算の単体テストを作成する
  - 閾値境界での識別性判定をテストする
  - 各色覚タイプでの色差計算が正しいことを確認する
  - _Requirements: 11.3, 11.4_

- [ ] 11.3 (P) [F] パレット全体の識別性検証を実装する
  - パレット内の全色ペアに対して各色覚タイプでの識別性を検証する
  - 隣接シェード間の識別性を優先的にチェックする
  - 背景色とテキスト色のペアを重点的に検証する
  - 問題のある色ペアと該当する色覚タイプを一覧化する
  - _Requirements: 11.5, 11.6_

- [ ] 11.4 [F] 改善提案生成を実装する
  - 識別困難な色ペアに対してHue調整の提案を生成する
  - Lightness調整による改善提案を生成する
  - Chroma調整による改善提案を生成する
  - 調整優先順位（Lightness→Hue→Chroma）に従った提案を生成する
  - _Requirements: 11.7_

- [ ] 11.5 [F] CVDスコア計算を実装する
  - パレット全体の色覚アクセシビリティスコア（0〜100）を計算する
  - 各色覚タイプでの識別可能率を加重平均する
  - スコアの内訳（色覚タイプ別）を表示する
  - _Requirements: 11.8_

- [ ] 11.6 [F] CVDシミュレーションUIを実装する
  - 各色覚タイプでのプレビュー表示切り替えを実装する
  - 識別困難な色ペアをハイライト表示する
  - CVDスコアと改善提案をUI上に表示する
  - demo.tsにCVDシミュレーション機能を統合する
  - _Requirements: 11.9_

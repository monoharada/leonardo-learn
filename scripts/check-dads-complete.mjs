...
(async () => {
  ...
  try {
    ...
    const pageInfo = await page.evaluate(() => {
      ...
      document.querySelectorAll('[class*="color"], [class*="token"], [class*="swatch"]').forEach(el => {
    ...
    const semanticTokens = pageInfo.colorTokens.filter(t =>
      t.text.includes('Success') || t.text.includes('Error') ||
      t.text.includes('Warning') || t.text.includes('green-') ||
      t.text.includes('red-')
    );
    ...
    const harmonyInfo = await page.evaluate(() => {
      ...
      document.querySelectorAll('*').forEach(el => {
        ...
        if (text && (text.includes('Complementary') || text.includes('補色') ||
                     text.includes('DADS') || text.includes('Analogous') ||
                     text.includes('類似色'))) {

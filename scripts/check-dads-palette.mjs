...
(async () => {
  ...
  try {
    ...
    const harmonyCards = await page.evaluate(() => {
      const cards = Array.from(document.querySelectorAll('.dads-card, [class*="harmony"]'));
    ...
    const semanticColors = await page.evaluate(() => {
      ...
      for (const selector of selectors) {
        ...
        elements.forEach(el => {
          const name = el.getAttribute('data-color-name') ||
          ...
          if (name && (name.includes('Success') || name.includes('Error') ||
                      name.includes('Warning') || name.includes('green') ||
                      name.includes('red'))) {
            results.push({
              name,
              token,
    ...
    console.log('- /Users/reiharada/dev/leonardo-learn/scripts/step1-after-generate.png');
    console.log('- /Users/reiharada/dev/leonardo-learn/scripts/step2-dads-selected.png');

...
| パレット | `src/ui/demo/views/palette-view.ts` | 生成された色を「UIの見え方」「トークン一覧」として整理して提示 | 擬似ページのプレビュー、トークン表（Primary/Accent + セマンティック）、背景色調整 |
...
- Primary/Accentだけでなく、リンクやSuccess/Warning/Errorなどの **セマンティックカラーの当て方** を提示する
...
- **トークン一覧（テーブル表示）**
  - Primary/Accent（生成した色）＋セマンティック（Link/Success/Warning/Error）を、トークン名・プリミティブ名・HEXで一覧できる
